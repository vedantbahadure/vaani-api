    ...
    <motion.div className="fixed inset-0 z-[100] bg-[#07090b] overflow-hidden" data-testid="cinematic-intro"
      ...
      >
        <img
          src="/vaani-logo.png"
          alt="VAANI"
          className="h-10 md:h-12 w-auto object-contain drop-shadow-[0_2px_16px_rgba(0,0,0,0.9)]"
        />
      </motion.div>

      {/* Skip */}
      <button onClick={onComplete} data-testid="skip-intro"
        className="absolute top-6 right-6 z-20 text-xs uppercase tracking-[0.2em] text-white/60 hover:text-white transition-colors duration-300 border border-white/15 bg-black/20 backdrop-blur-sm rounded-full px-4 py-2 hover:border-white/30">
        {t(lang, "skip")}
      </button>
